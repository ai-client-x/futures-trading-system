...
def run_backtest():
    ...
    for i, date in enumerate(trade_dates):
        ...
        # 每10天判断一次市场状态（减少计算量）
        if i % 10 == 0:
            last_regime = get_market_regime_v2(date)
        ...
        for pos in list(positions):
            ...
            if curr_ret >= TP3:
                sell_qty = pos['qty']
                reason = '止盈20%清仓'
            elif curr_ret >= TP2 and pos.get('tp15_triggered', False) == False:
                sell_qty = int(pos['qty'] * 0.6)
                reason = '止盈15%卖出60%'
                pos['tp15_triggered'] = True
            elif curr_ret >= TP1 and pos.get('tp10_triggered', False) == False:
        ...
        for pos, price, sell_qty, reason in to_sell:
            ...
            regime_trades[last_regime.value] += 1
                ...
                if signal == 'buy':
                    ...
                    if qty > 0 and capital > price * qty * 1.001:
                        ...
                        positions.append({
                            ...
                            'layers': 1,
                            'name': cand.get('name', ''),
                            'tp10_triggered': False,  # 10%止盈是否已触发
                            'tp15_triggered': False,  # 15%止盈是否已触发
                        })
    ...
    print("="*60)
    print("自适应市场状态交易系统 V2 (2020-2024)")
    print("="*60)
