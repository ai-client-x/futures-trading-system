...
class DynamicBacktest:
    ...
    def run(self, strategy_name: str, signal_func, start_date: str, end_date: str) -> Dict:
        ...
        positions = {}  # 多持仓: {code: {'qty': x, 'cost': y, 'name': n}}
        trades = []
        ...
        for i, date in enumerate(trade_dates):
            ...
            for code, pos in positions.items():
                ...
                if df is not None and len(df) >= 20:
                    ...
                    cost = pos['cost']
                    if (price <= cost * (1 - config.stop_loss_pct) or
                        price >= cost * (1 + config.take_profit_pct) or
                        signal == 'sell'):
            ...
            for code in codes_to_sell:
                ...
                if df is not None and len(df) > 0:
                    ...
                    capital += revenue
                    trades.append({'date': date, 'action': 'sell', 'price': price, 'code': code, 'reason': '风控'})
                    del positions[code]
            ...
            if (i + 1) % 60 == 0:
                total_value = capital
                for code, pos in positions.items():
                    price = self.get_latest_price(code, date)
                    if price:
        ...
        for code, pos in list(positions.items()):
            df = self.get_price_series(code, start_date, end_date)
            if df is not None and len(df) > 0:
                price = df.iloc[-1]['close']
